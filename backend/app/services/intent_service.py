...
SYSTEM_PROMPT = """
You are an EMS (Employee Management System) issue tracker AI.
Extract structured issue details from user input and return ONLY valid JSON.
...
Return this exact JSON structure:
{
  ...
  "fields": {
    "issue_number": null,
    "title": "Clear 5-7 word title",
    "description": "Detailed explanation — what is happening, where, and expected behavior. Minimum 2 sentences.",
    "issue_type": "issue",
    "module": "Module name if mentioned",
    "priority": "medium",
    "observations": ["specific observation 1", "specific observation 2"],
...
ACTION RULES:
- add/create/log/note/new/report → "create_issue"
- update/change/edit/modify → "update_issue"
- resolved/fixed/done/close/completed → "resolve_issue"
- Unclear → "unknown", needs_clarification: true

FIELD RULES:
- title: max 7 words, Title Case
- description: detailed, minimum 2 sentences, never repeat title
- issue_type: "issue" if broken, "feature" if new functionality
- module: Login/Dashboard/Navbar/Sidebar/Attendance/Payroll/Members/Certificate/Finance/Reports/Settings
- priority: "low"/"medium"/"high"/"critical"
- observations: specific symptoms, empty [] if none
- issue_number: only when user says "issue 3" or "#5"
...
# ─── Provider 3: OpenRouter (last fallback) ───────────────────────────────────
...
# # ─── Main call_ai — tries providers in order ──────────────────────────────────
...
def parse_intent(request: IntentRequest) -> IntentResponse:
    try:
        ...
        intent = ParsedIntent(
            ...
            confidence=data.get("confidence", 0.5),
            raw_transcript=request.transcript,
            confirmation_message=data.get("confirmation_message") or f"Understood: {data.get('action', 'action')}",
            needs_clarification=data.get("needs_clarification", False)
        )
        return IntentResponse(success=True, intent=intent)
        ...
        # Retry with simpler prompt
        try:
            simple = f'Return JSON only. Transcript: "{request.transcript}". Fields: action (create_issue/update_issue/resolve_issue/unknown), title, description.'
            raw2   = _clean_json(call_ai(simple))
            data   = json.loads(raw2)
            intent = ParsedIntent(
                action=data.get("action", "unknown"),
                fields=IssueFields(**{
                    k: v for k, v in data.get("fields", {}).items()
                    if k in IssueFields.model_fields
                }),
                confidence=0.5,
                raw_transcript=request.transcript,
                confirmation_message=data.get("confirmation_message", f"Understood: {request.transcript[:50]}"),
            ...
            return IntentResponse(success=False, error="Could not understand. Please try again.")
